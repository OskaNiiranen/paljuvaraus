...
// Tämä komponentti sisältää kaiken logiikan ja näkymän
function KassaContent() {
  const searchParams = useSearchParams();

  // Luetaan parametrit URL:sta
  const fromDateStr = searchParams.get("from");
  const toDateStr = searchParams.get("to");
  const priceStr = searchParams.get("price");

  // Muotoillaan päivämäärät suomalaiseen muotoon
  const formatDate = (dateString: string | null) => {
    if (!dateString) return "Ei valittu";
    return new Date(dateString).toLocaleDateString("fi-FI");
  };

  // Rakennetaan bookingDetails-olio URL:sta luetuilla tiedoilla
  const bookingDetails = {
    productName: "Käsinrakennettu Paljukärrymme",
    startDate: formatDate(fromDateStr),
    endDate: formatDate(toDateStr),
    totalPrice: priceStr ? parseInt(priceStr, 10) : 0,
  };
...
// Tämä on välttämätöntä, koska useSearchParams on client-puolen hook.
